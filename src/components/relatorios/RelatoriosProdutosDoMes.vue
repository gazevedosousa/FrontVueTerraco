<template>
    <div class="q-pa-md">
      <q-table
        :title="`Relatório de Produtos - Mês ${formatMonth((mes as string))}`"
        :rows="relatorio"
...
<script setup lang="ts">
    ...
    import { formatMonth, formatPrice } from 'src/services/utils/utils';
    ...
    const dowloadPdf = () => {
      //@ts-ignore
      const table = produtosPdfTable(formatMonth((mes as string)), relatorio.value, vrTotal.value);
                
      html2pdf(table, {
        filename: `Relatorio Produtos - ${formatMonth((mes as string))}`
      });
    }
</script>