<template>
    <div class="q-pa-md">
      <q-table
        :title="`Relatório de Pagamentos - Mês ${formatMonth((mes as string))}`"
        :rows="relatorio"
...
<script setup lang="ts">
    ...
    import { formatDate, formatMonth, formatPrice } from 'src/services/utils/utils';
    ...
    api.get(`api/pagamento/porMes/${mes}`)
        .then((response => {
    ...
    const dowloadPdf = () => {
      //@ts-ignore
      const table = pagamentosPdfTable(formatMonth((mes as string)), relatorio.value, vrTotal.value);
                
      html2pdf(table, {
        filename: `Relatorio Pagamentos - ${formatMonth((mes as string))}`
      });
    }
</script>